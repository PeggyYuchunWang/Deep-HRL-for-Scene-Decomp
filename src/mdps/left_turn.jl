...
function POMDPs.convert_s(ts::Type{Scene}, v::V, mdp::DrivingLeftTurnMDP) where V<:AbstractArray
    ...
    lane2 = v[15] == 1 ? LaneTag(3,1) : LaneTag(1,1)
    state2 = VehicleState(Frenet(mdp.roadway[lane2], v[10]*mdp.road_length, v[11]*mdp.lane_width), mdp.roadway, v[12]*mdp.speed_limit)
    veh2 = Entity(state2, def, 2)


    lane3 = v[24] == 1 ? LaneTag(3,1) : LaneTag(1,1)
    state3 = VehicleState(Frenet(mdp.roadway[lane3], v[19]*mdp.road_length, v[20]*mdp.lane_width), mdp.roadway, v[21]*mdp.speed_limit)
