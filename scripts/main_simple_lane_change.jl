includet("../src/AutomotiveHRLSceneDecomp.jl")
includet("../src/mdps/simple_two_lane.jl")
includet("../src/utils/helpers.jl")
# using AutomotiveHRLSceneDecomp

mdp = DrivingMDP()
model = Chain(Dense(15, 32, relu), Dense(32, 32, relu), Dense(32, n_actions(mdp)))

solver = DeepQLearningSolver(qnetwork = model, max_steps=400_000,
                             ...
                             logdir="log/simple_lane_test5/", batch_size=128)
...
# @save "weights/simple_lanechange_policy_weights_test2.jld2" weights
# @load "weights/simple_lanechange_policy_weights_test2.jld2" weights
